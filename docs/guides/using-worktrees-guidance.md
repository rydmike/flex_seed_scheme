...
```bash
git worktree add ../fss-feature -b feature/x   # new branch
git worktree add ../fss-hotfix hotfix/y        # existing branch
git worktree list
git worktree remove ../fss-feature
```
...
```bash
claude --worktree flex-tones      # or -w
...
```
/worktree fix the failing FlexTones tests
/apply-worktree      # pull the changes into your main checkout to test
/delete-worktree     # clean up
/best-of-n sonnet,gpt,composer fix the flaky ColorScheme test
```
...
```bash
cd .claude/worktrees/flex-tones
git add -A && git commit -m "..."
git push -u origin worktree-flex-tones
gh pr create --fill
```
...
```bash
claude --cloud "Fix the flaky test in flex_tones_test.dart"
...
- **Example iOS uses CocoaPods** (`example/ios/Podfile`). Do not run `pod install` yourself; the first `flutter run` in `example/` does that. Budget a few extra minutes for that first iOS example build.
