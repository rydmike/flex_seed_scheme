...
## Deliberate comment divergences from upstream

These upstream comment defects were fixed on purpose in the fork (FSS 5.0.0 doc sweep). When a sync shows a comment-only diff on these lines, keep the fork's version — do not "restore" upstream's text:

- `hct/cam16.dart` — upstream duplicates the 15-line `Cam16` class doc verbatim; fork keeps one copy. `fromJchInViewingConditions` doc uses `[J]` (upstream `[j]` does not resolve).
- `utils/math_utils.dart` — `clampDouble` doc says "floating-point number", not upstream's "integer".
- `utils/color_utils.dart` — `yFromLstar`/`lstarFromY` docs: upstream inverts linear/logarithmic; fork says L\* is perceptually uniform (nonlinear) and Y is linear.
- `palettes/tonal_palette.dart` — class doc: ARGB (upstream "ARBG") and key color deduced from highest-chroma input (upstream claims "black").
- `dynamiccolor/src/contrast_curve.dart` — 0.13.0's broadened "value that changes with the contrast level" wording, plus plain field docs (upstream self-links `[low]` on `low`).
- `dynamiccolor/src/tone_delta_pair.dart` ("difference"), `contrast/contrast.dart` ("the the"), `score/score.dart` ("undesirable"), `dislike/dislike_analyzer.dart` ("distaste"), `dynamiccolor/dynamic_color.dart` ("it is usually"; `getArgb` doc rejoined) — typo fixes.
- `scheme/scheme.dart` — upstream "This file is automatically generated" banner removed (the file is heavily hand-modified in this fork).
- `palettes/key_color.dart` — FSS-extracted file: carries the Google license header, trimmed `create()` doc (no `[hue]`/`[requestedChroma]` params), tone-keyed cache doc.
- `dynamiccolor/variant.dart` — FSS-added `///` docs on `rainbow`/`fruitSalad` state the actual hue behavior; the upstream `description` string data is intentionally unchanged.
